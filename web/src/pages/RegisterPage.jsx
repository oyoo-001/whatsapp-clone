...
const COUNTRY_CODES = {
  KE: '+254', NG: '+234', ZA: '+27', TZ: '+255',
  UG: '+256', RW: '+250', GH: '+233', CM: '+237',
  US: '+1', GB: '+44', IN: '+91', PK: '+92',
  BD: '+880', PH: '+63', VN: '+84', TH: '+66',
  ID: '+62', MY: '+60', SG: '+65', AE: '+971',
  SA: '+966', EG: '+20', MA: '+212', DZ: '+213',
  FR: '+33', DE: '+49', IT: '+39', ES: '+34',
  PT: '+351', NL: '+31', BE: '+32', CH: '+41',
  SE: '+46', NO: '+47', DK: '+45', FI: '+358',
  PL: '+48', RO: '+40', GR: '+30', TR: '+90',
  RU: '+7', UA: '+380', BR: '+55', AR: '+54',
  MX: '+52', CO: '+57', CL: '+56', PE: '+51',
  CN: '+86', JP: '+81', KR: '+82', AU: '+61',
  NZ: '+64',
};
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: `linear-gradient(145deg, ${Colors.primary} 0%, #00695C 100%)` }}>
      <div style={{ padding: '20px 16px 0', animation: 'fadeIn 0.4s ease'  }}>
        <button onClick={() => navigate(-1)} style={{
          ...
          width: 40, height: 40, cursor: 'pointer', color: Colors.white,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          backdropFilter: 'blur(8px)', transition: 'background 0.2s',
        }}><ArrowLeft size={20} /></button>
      </div>
      <div style={{ padding: '20px 24px 28px', animation: 'fadeInUp 0.5s ease' }}>
        <h1 style={{ color: Colors.white, fontSize: 28, margin: '0 0 6px', fontWeight: 800, letterSpacing: '-0.5px' }}>Create Account</h1>
        <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 15, margin: 0 }}>Join WhatsApp Clone today</p>
      </div>

      <form onSubmit={handleSubmit} style={{
        background: Colors.white, borderRadius: '32px 32px 0 0', padding: '32px 28px 48px',
        ...
        <div style={fieldGroup}>
          ...
          <div style={inputWrapper}>
            ...
            <input value={form.username} onChange={update('username')} placeholder="Your display name" style={inputField} />
          </div>
        </div>

        <div style={fieldGroup}>
          <label style={fieldLabel}>
            PHONE NUMBER {detecting && <span style={{ fontSize: 10, color: '#B0BEC5', fontWeight: 400, marginLeft: 4 }}>(detecting...)</span>}
          </label>
          <div style={{ display: 'flex', gap: 10 }}>
            <div style={{ position: 'relative' }}>
              <button type="button" onClick={() => setShowCodePicker(!showCodePicker)} style={{
                height: 54, background: '#F5F7FA', border: '2px solid transparent',
                borderRadius: 14, padding: '0 12px', cursor: 'pointer', display: 'flex',
                alignItems: 'center', gap: 4, fontSize: 15, fontWeight: 600, color: Colors.textPrimary,
              ...
              {showCodePicker && (
                <div style={{
                  ...
                  background: Colors.white, borderRadius: 14, boxShadow: '0 8px 30px rgba(0,0,0,0.15)',
                  maxHeight: 200, overflowY: 'auto', marginBottom: 4, padding: 4,
                  animation: 'fadeInUp 0.15s ease',
                }}>
                  {uniqueCodes.map((code) => (
                    <button key={code} type="button" onClick={() => { setCountryCode(code); setShowCodePicker(false); }} style={{
                      display: 'block', width: '100%', padding: '10px 12px',
                      background: code === countryCode ? '#F0F2F5' : 'none',
                      border: 'none', borderRadius: 8, fontSize: 14, fontWeight: code === countryCode ? 700 : 400,
                      cursor: 'pointer', color: Colors.textPrimary, textAlign: 'center',
                    }}>{code}</button>
                  ))}
                </div>
              )}
            ...
            <div style={{ flex: 1, position: 'relative' }}>
              ...
              <input value={form.phoneNumber} onChange={update('phoneNumber')} placeholder="712345678" type="tel" style={{ ...inputField, paddingLeft: 48, height: 54 }} />
            </div>
          </div>
        </div>
        ...
        <div style={fieldGroup}>
          ...
          <div style={inputWrapper}>
            ...
            <input value={form.password} onChange={update('password')} placeholder="At least 6 characters" type="password" style={inputField} />
          </div>
        </div>
...
const fieldLabel = {
  ...
  letterSpacing: '0.8px', marginBottom: 8, display: 'block',
};
...
const inputField = {
  ...
  borderRadius: 14, padding: '0 16px 0 48px', fontSize: 15, color: Colors.textPrimary,
...
const linkText = {
  textAlign: 'center', color: Colors.textSecondary, fontSize: 14, margin: '8px 0 0',
};
