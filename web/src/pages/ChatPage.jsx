...
import { usersAPI, uploadAPI, callsAPI } from "../services/api";
...
const ChatPage = () => {
  ...
  const isBroadcast = chatUser?.id === 0;
  ...
  const sameDay = (a, b) =>
    getDayStart(new Date(a)).getTime() === getDayStart(new Date(b)).getTime();
  ...
  useEffect(() => {
    ...
    if (chatPin) {
      const lockedList = JSON.parse(
        localStorage.getItem("lockedChats") || "[]",
      );
      if (
        lockedList.includes(uid) &&
        sessionStorage.getItem(`unlocked_${uid}`) !== "true"
      ) {
        setLocked(true);
        return;
      }
  ...
  useEffect(() => {
    ...
    const u3 = socketService.on("broadcast:new", ({ broadcast }) => {
      if (uid === 0) {
        addMessage({
          id: `bc-${broadcast.id}`,
          senderId: 0,
    ...
    const u4 = socketService.on("broadcast:deleted", ({ broadcastId }) => {
      if (uid === 0) {
  ...
  const handleDeleteConfirm = async () => {
    if (!contextMessage) return;
    try {
      await deleteMessageAction(contextMessage.id, deleteMode, contextMessage);
      toast(
        deleteMode === "all" || contextMessage.isBroadcast
  ...
  const handleSend = async (overrideContent) => {
    const content =
      overrideContent !== undefined ? overrideContent : text.trim();
    ...
    try {
      ...
      if (msg?.includes("blocked"))
        toast("Cannot send — user is blocked", "error");
  ...
  const startCall = async (type) => {
    const channelName = `chat-${uid}-${Date.now()}`;
    const startedAt = Date.now();
    let callLogId = null;
    try {
      const { data } = await callsAPI.initiateCall({ receiverId: uid, callType: type });
    ...
    navigate(`/call/${channelName}`, {
      state: {
        ...
        remoteAvatar: chatUser?.avatar || null,
      },
    });
  };
  ...
  const addToContacts = async () => {
    try {
      ...
      toast("Already in contacts", "info");
    }
  ...
  const toggleBlock = async () => {
    try {
      ...
      toast("Failed to toggle block", "error");
    }
    setShowActions(false);
    ...
    return (
      ...
      >
        <div style={{ textAlign: "center", padding: 40 }}>
          ...
          />
          <h2 style={{ color: Colors.white, fontSize: 20, marginBottom: 8 }}>
            Chat Locked
          </h2>
          <p style={{ color: "#B2DFDB", fontSize: 13, marginBottom: 20 }}>
          ...
          />
          {pinError && (
            <p style={{ fontSize: 12, color: "#FF8A80", marginTop: 8 }}>
              {pinError}
            </p>
          )}
          <div
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          onClick={() => !isBroadcast && setShowProfile(true)}
          style={{
            width: 38,
            height: 38,
            borderRadius: "50%",
        ...
        >
         ...
         <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {(chatUser?.id === 0 || chatUser?.isVerified) && <Verified size={14} color={Colors.accent} style={{ flexShrink: 0 }} />}
          </div>
        ...
        {!isBroadcast && (
          <button onClick={() => startCall("voice")} style={iconBtn} title="Voice call">
            <Phone size={18} />
          </button>
        )}
        {!isBroadcast && (
          <button onClick={() => startCall("video")} style={iconBtn} title="Video call">
            <Video size={18} />
          </button>
        )}
        {!isBroadcast && (
      ...
      {showActions && (
        ...
        >
          ...
          {localStorage.getItem("chatPin") && (
            <button
              onClick={() => {
                ...
                if (lockedList.includes(uid)) unlockChat();
                else lockChat();
                setShowActions(false);
              }}
              style={menuItem}
            >
              <Lock size={16} />
              {JSON.parse(localStorage.getItem("lockedChats") || "[]").includes(
                uid,
              )
                ? "Unlock Chat"
                : "Lock Chat"}
          ...
          <button
            onClick={toggleBlock}
            style={{ ...menuItem, color: Colors.red }}
          >
      ...
      >
        {isLoadingMessages ? (
          ...
          >
            ...
            <p style={{ fontSize: 14 }}>
              {isBroadcast ? "No broadcasts yet" : "Send a message to start chatting"}
            </p>
            <p style={{ fontSize: 12, marginTop: 8, opacity: 0.6 }}>
              {isBroadcast ? "TuChat broadcasts will appear here" : "🔒 End-to-end encrypted"}
            </p>
          </div>
        ) : (
          <>
            ...
            {messages.map((msg, i) => {
              const showDate =
                i === 0 || !sameDay(msg.createdAt, messages[i - 1].createdAt);
              return (
                <div
                  key={msg.id}
                  style={{ animation: `fadeInUp 0.2s ease ${i * 0.02}s both` }}
                >
                  {showDate && (
                    <div
                      style={{
            ...
            <span style={{ display: "flex", gap: 2 }}>
              ...
              <span
                style={{
                  ...
                  borderRadius: "50%",
                  background: Colors.textSecondary,
                  animation: "pulse 0.8s ease 0.2s infinite",
                }}
              />
              <span
                style={{
                  ...
                  borderRadius: "50%",
                  background: Colors.textSecondary,
                  animation: "pulse 0.8s ease 0.4s infinite",
                }}
              />
            </span>
      ...
      <MessageContextMenu
        ...
        isMine={contextMessage?.senderId === currentUser.id}
        position={contextPos}
                ...
                style={{
                  ...
                  border:
                    deleteMode === "me"
                      ...
                      : "2px solid #E0E0E0",
                  background: deleteMode === "me" ? "#E8F5E9" : Colors.white,
                  cursor: "pointer",
                  textAlign: "center",
                ...
                style={{
                  ...
                  border:
                    deleteMode === "all"
                      ? `2px solid ${Colors.primary}`
                      : "2px solid #E0E0E0",
                  background: deleteMode === "all" ? "#E8F5E9" : Colors.white,
                  cursor: "pointer",
                  textAlign: "center",
                  transition: "all 0.15s",
                }}
      ...
      {!isBroadcast && (replyTo || editMessage) && (
        <div
          style={{
            background: Colors.lighterGrey,
            padding: "8px 12px",
        ...
        >
          <div
            style={{
              color: editMessage ? Colors.secondary : Colors.accent,
              display: "flex",
            }}
          >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              {editMessage
                ? "Editing"
                : `Replying to ${replyTo?.senderId === currentUser.id ? "yourself" : chatUser?.username || "..."}`}
            </div>
            <div
              style={{
                fontSize: 13,
      ...
      {previewFile && (
        ...
        >
          ...
          <button
            ...
            disabled={uploading}
            style={{
              background: uploading ? Colors.textHint : Colors.primary,
              border: "none",
              borderRadius: "50%",
              width: 38,
              height: 38,
        ...
        !isBlocked && (
          ...
          >
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  padding: 4,
                  color: showEmoji ? Colors.primary : Colors.textHint,
                  display: "flex",
                  marginBottom: 6,
                }}
              >
            ...
            {text.trim() ? (
              ...
              <button onClick={() => setShowVoice(true)} style={inputBtn}>
                <Mic size={20} />
              </button>
            )}
          </div>
